...
import com.toceansoft.common.utils.CommonUtil;
...
public class AliPayServiceImpl implements IAliPayService {
	...
	public String aliPay(Product product) {
		...
		AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
				...
				.setSellerId(sellerId).setBody(body)// 128长度 --附加信息
		...
		AlipayF2FPrecreateResult result = AliPayConfig.getAlipayTradeService()
				.tradePrecreate(builder);
	...
	public String aliCloseorder(Product product) {
		...
		String message = Constants.SUCCESS;
		try {
			String imgPath = Constants.QRCODE_PATH + Constants.SF_FILE_SEPARATOR + "alipay_"
					+ product.getOutTradeNo() + ".png";
			...
			if (file.exists()) {
				...
				if (response.isSuccess()) {// 扫码未支付的情况
					...
					if ("ACQ.TRADE_NOT_EXIST".equals(response.getSubCode())) {
						...
						logger.info("订单号：" + product.getOutTradeNo() + "支付宝关闭订单失败"
	...
	public boolean rsaCheckV2(Map<String, String> params) {
		...
		boolean signVerified = false;
		try {
			signVerified = AlipaySignature.rsaCheckV2(params, Configs.getAlipayPublicKey(),
					"UTF-8");
		} catch (AlipayApiException e) {
			e.printStackTrace();
		}
